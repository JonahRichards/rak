...
convert_data = False
process_data = False # Relevant calculations for your scientific case
create_plots = False
show_plots = False
...
def main():
    ...
    if load_data:
        ...
        data_file = f"{root}\Raw Data\Raks_tm_readout.csv"
        ...
        raw_data["t"] = [t - t_0 for t in raw_data["t"]]

        # Sanitize GPS-data
        if sanitize_gps:
            mask = raw_data['raw_satellites'] < 3
            mask2 = raw_data['raw_satellites'] == 3
    ...
    if export_data:
        ...
        if drop_raw:
            analogue.drop(columns=[col for col in analogue.columns if "raw_" in col], inplace=True)
            gps.drop(columns=[col for col in gps.columns if "raw_" in col], inplace=True)
            imu.drop(columns=[col for col in imu.columns if "raw_" in col], inplace=True)
            temp_array.drop(columns=[col for col in temp_array.columns if "raw_" in col], inplace=True)
            power_sensor.drop(columns=[col for col in power_sensor.columns if "raw_" in col], inplace=True)
            misc.drop(columns=[col for col in misc.columns if "raw_" in col], inplace=True)
